...
pkg update -y
pkg install zsh fastfetch --color always git curl lsd bat zoxide fzf cmatrix tmux -y
...
cd ~/termux-dotfiles
cp zsh/.zshrc ~/.zshrc
mkdir -p ~/.config/fastfetch --color always
cp config/fastfetch --color always/config.jsonc ~/.config/fastfetch --color always/config.jsonc
mkdir -p ~/.termux
